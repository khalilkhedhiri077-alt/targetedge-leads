...
const S = {
  ...
  opCard: (sel) => ({
    border: sel ? '2px solid #185FA5' : '1px solid #ddd',
    borderRadius: 10, padding: '14px 10px', textAlign: 'center', cursor: 'pointer',
    background: sel ? '#E6F1FB' : '#fafafa', transition: 'all 0.12s'
  }),
  ...
  input: (err) => ({
    ...
    border: err ? '1px solid #c00' : '1px solid #ddd',
    borderRadius: 7, background: '#fafafa', outline: 'none'
  }),
  textarea: (err) => ({
    width: '100%', padding: '8px 10px', fontSize: 13,
    border: err ? '1px solid #c00' : '1px solid #ddd',
    borderRadius: 7, background: '#fafafa', resize: 'vertical', minHeight: 70, outline: 'none'
  }),
  err: { fontSize: 10, color: '#c00', marginTop: 3 },
  radioGroup: { display: 'flex', flexDirection: 'column', gap: 6 },
  radioOpt: (sel) => ({
    display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px',
    border: sel ? '1.5px solid #185FA5' : '1px solid #ddd',
    borderRadius: 7, cursor: 'pointer', background: sel ? '#E6F1FB' : '#fafafa'
  }),
  ...
  audioZone: (hasFile) => ({
    border: hasFile ? '2px solid #3B6D11' : '2px dashed #ddd',
    borderRadius: 10, padding: 22, textAlign: 'center', cursor: 'pointer',
    background: hasFile ? '#EAF3DE' : '#fafafa', marginBottom: 8
  }),
  audioErr: { fontSize: 11, color: '#c00', marginTop: 5 },
...
export default function FormulaireSaisie({ centre }) {
  ...
  async function submit() {
    ...
    try {
      ...
      body.append('centre', centre);
      body.append('operation', op);
      body.append('data', JSON.stringify({ ...form, qualification: form.qualification }));
      if (audioFile) body.append('audio', audioFile);
      await fetch('/api/leads/create', { method: 'POST', body });
      setDone(true);
    } catch (e) {
  ...
  function renderQField(f) {
    ...
    if (f.type === 'checkboxes') {
      ...
      return (
        <div key={f.id} style={S.fg}>
          <label style={S.label}>{f.label}{f.req && <span style={S.req}>*</span>}<span style={S.hint}>(plusieurs choix possibles)</span></label>
          {f.options.map(opt => (
  ...
  if (done) return (
    <div style={S.app}>
      ...
      <div style={S.success}>
        ...
        <div style={{ fontSize: 13, color: '#555', lineHeight: 1.6 }}>En attente de validation TargetEdge.<br />Vous serez notifié du résultat.</div>
      ...
      <div style={S.body}>
        ...
        {step === 6 && (
          <>
            ...
            <div style={S.warnBox}>Notez tout ce qui ne figure pas dans les champs : attitude du prospect, hésitations, engagement verbal, contexte particulier, rappel urgent...</div>
            <div style={S.fg}>
              <label style={S.label}>Commentaire<span style={S.req}>*</span></label>
              <textarea style={S.textarea(errors.commentaire)} placeholder="Ex. : Très motivé, renouvellement dans 6 semaines, préfère être rappelé le matin..." value={form.commentaire} onChange={e => setF('commentaire', e.target.value)} />
        ...
        {step === 7 && (
          <>
            ...
            <label style={S.audioZone(!!audioFile)}>
              <input type="file" accept="audio/*,.mp3,.wav,.m4a,.ogg" style={{ display: 'none' }} onChange={e => { if (e.target.files[0]) { setAudioFile(e.target.files[0]); setErrors(er => ({ ...er, audio: '' })); } }} />
              {audioFile ? (
                <>
                ...
                <>
                  ...
                  <div style={{ fontSize: 13, color: '#555', marginTop: 6 }}>Cliquer pour importer l'enregistrement</div>
                  <div style={{ fontSize: 11, color: '#999', marginTop: 3 }}>MP3, WAV, M4A · max 100 Mo</div>
                </>
              )}
            </label>
            {errors.audio && <div style={S.audioErr}>{errors.audio}</div>}
            <div style={S.btnRow}>
